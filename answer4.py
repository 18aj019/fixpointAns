...
#Ipv4抽出
is_dup = df.duplicated(subset=["ipv4"])#IP抽出
...
def func5(ip_list):
    #サブネットからのネットワークアドレス算出
    ip_list2 = []
    for i in range(len(ip_list)):
        #print(ipaddress.IPv4Interface(ip_list[i]).network)
        ip_list2.append(str(ipaddress.IPv4Interface(ip_list[i]).network))
    return list(set(ip_list2))
